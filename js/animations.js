export function initAnimations() {
  ...
  if (heroBadge) {
    ...
    heroTl
      ...
      .fromTo('.hero-title', { opacity: 0, y: 40 }, { opacity: 1, y: 0, duration: 1.2 }, 0.5)
      .fromTo('.hero-sub', { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.8 }, 0.8)
      .fromTo('.hero-cta', { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.8 }, 1.0);
  }
  ...
  gsap.utils.toArray('.origin-card, .coffee-card, .pc, .partner-card').forEach((card, i) => {
    gsap.fromTo(card,
      { opacity: 0, y: 60 },
      {
        ...
        opacity: 1, y: 0, duration: 0.8, delay: i % 3 * 0.15, ease: 'power3.out',
      }
    );
  });
